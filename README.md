...
このAPIのリクエストボディは `serviceId` / `workspaceName` / `customWorkspaceType` の3項目のみで、**サービス名だけで新規作成することはできません**（必ず `serviceId` が必要）。

組織にまだ無いサービス（例: Tailscale, Vercel）を登録したい場合も、**そのサービスの標準 `serviceId`**（Adminaのサービスマスター上のID）を指定します。作成に成功すると、組織にそのサービスが追加され、出力の `isNewService` が `true` になります。

> 標準サービスの `serviceId` は、メニュー **「サービス一覧を取得」** または Admina 管理画面で確認します。完全に独自（マスターに存在しない）サービスを新規作成する場合は、別途カスタムサービス作成の手続きが必要です。
